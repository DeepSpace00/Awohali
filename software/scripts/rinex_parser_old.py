# Python Script for converting a RINEX file to a generic csv according to given format specifiers
...
#   P2 is the P code measurement on the L2 band in compliance with the RINEX
#   2.11 specification.
...
rinFileName = "../data/RINEX_data/ORMD/2025-11-25/ormd3290.25o"
csvFileName = "../data/RINEX_data/ORMD/2025-11-25/ormd3290.csv"
writeMode = "w"

csvFields = ["time", "prn", "C1", "P1", "L1", "D1", "S1", "C2", "P2", "L2", "D2", "S2", "C5", "L5", "D5", "S5"]
rinFields = []
...
if argc < 2:
    ...
    if sys.argv[1] == "-h" or sys.argv[1] == "--help":  # help
        print("rinex_parser_old.py: Python utility for converting RINEX to CSV")
        print("Usage:")
        print("rinex_parser_old.py [RINEX_File [CSV_File]] [-al] [-f Fields]")
        ...
        print("  -l         : Legacy data only, supersedes -f")
        ...
        print("    P2 is the P code measurement on the L2 band in compliance with the RINEX")
        print("    2.11 specification.")
        exit(0)
    ...
    while i < argc:
        ...
        if sys.argv[i] == "-f":  # Use the user specified format
            ...
            csvFields = ["time", "prn", "C1", "P1", "L1", "D1", "S1", "S2", "C2", "L2", "D2"]
...
while currentLine != "":
    if currentLine[27:30] == " 0 ":
        ...
        for prn in epochPRN:
            gpsRecord["prn"] = prn
            num_full_lines = numFields // 5
            for j in range(num_full_lines):
                currentLine = rinFile.readline()
                for i in range(5):
            ...
            for i in range(numFields % 5):
                record = currentLine[i * 16 + 1:i * 16 + 14].lstrip()
                if record == "":
                    gpsRecord[rinFields[num_full_lines * 5 + i]] = "0"
                else:
                    gpsRecord[rinFields[num_full_lines * 5 + i]] = record
