...
void parse_extension_info(const zedf9p_mon_ver_t *version) {
    ...
    for (uint8_t i = 0; i < version->num_extensions; i++) {
        ...
        // GNSS constellations
        else if (strstr(ext, "GPS") != nullptr || strstr(ext, "GLO") != nullptr ||
                 strstr(ext, "GAL") != nullptr || strstr(ext, "BDS") != nullptr) {
            Serial.print("GNSS Support:     ");
            Serial.println(ext);
        }

        // Additional capabilities
        else if (strstr(ext, "SBAS") != nullptr || strstr(ext, "QZSS") != nullptr ||
                 strstr(ext, "L2C") != nullptr || strstr(ext, "L5") != nullptr) {
            Serial.print("Capabilities:     ");
            Serial.println(ext);
        }

        // Firmware date/build info
        else if (strstr(ext, "EXT CORE") != nullptr) {
