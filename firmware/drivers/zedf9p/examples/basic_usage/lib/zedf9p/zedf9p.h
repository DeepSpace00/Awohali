 ...
 * @details Derived from SparkFun_u-blox_GNSS_Arduino_Library
 */
...
typedef enum {
  ...
  SFE_UBLOX_PACKET_NOT_ACKNOWLEDGED // This indicates that we received a NACK
} sfe_ublox_packet_validity_e;

/**
 * @brief Identify which packet buffer is in use:
 *
 * @details packetCfg (or a custom packet), packetAck or packetBuf
 * @details packetAuto is used to store expected "automatic" messages
*/
typedef enum {
...
/**
 * @brief Define a struct to allow selective logging / processing of NMEA messages
 *
 * @details Set the individual bits to pass the NMEA messages to the file buffer and/or processNMEA
 * @details Setting bits.all will pass all messages to the file buffer and processNMEA
 */
typedef struct {
  union {
    uint32_t all;
...
} sfe_ublox_sentence_types_e;


typedef struct {
  bool expectACKonly;   // default false
  bool alwaysPrintPayload; // default false
  bool push_raw_data_stop; // default false
  sfe_ublox_mga_assist_ack_e mgaAck; // default SFE_UBLOX_MGA_ASSIST_ACK_NO
} zedf9p_settings_t;

typedef struct {
  bool disable_7f_check; // default true

} zedf9p_ubx_settings_t;
...
zedf9p_status_t setPacketCfgPayloadSize(size_t payloadSize); // Set packetCfgPayloadSize
...
zedf9p_status_t setMaxNMEAByteCount(int8_t newMax);
int8_t getMaxNMEAByteCount(void);

// Returns true if device answers on _gpsI2Caddress address or via Serial
zedf9p_status_t isConnected(uint16_t maxWait);


zedf9p_status_t disableDebugging(void);                       // Turn off debug statements
zedf9p_status_t debugPrint(char *message);                    // Safely print debug statements
zedf9p_status_t debugPrintln(char *message);                  // Safely print debug statements
...
zedf9p_status_t disableUBX7Fcheck(bool disable_7f_check); // When logging RAWX data, we need to be able to disable the "7F" check in checkUbloxI2C

zedf9p_status_t checkUbloxI2C(ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID);    // Method for I2C polling of data, passing any new bytes to process()
zedf9p_status_t checkUbloxSerial(ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID); // Method for serial polling of data, passing any new bytes to process()
zedf9p_status_t checkUbloxSpi(ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID);    // Method for spi polling of data, passing any new bytes to process()

// Process the incoming data

zedf9p_status_t process(uint8_t incoming, ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID);             // Processes NMEA and UBX binary sentences one byte at a time
zedf9p_status_t processNMEA(char incoming) __attribute__((weak));                                                           // Given a NMEA character, do something with it. User can overwrite if desired to use something like tinyGPS or MicroNMEA libraries
zedf9p_status_t processNMEA_v(char incoming);                                                                       // Given a NMEA character, do something with it. User can overwrite if desired to use something like tinyGPS or MicroNMEA libraries
...
sfe_ublox_sentence_types_e processRTCMframe_v(uint8_t incoming, uint16_t *rtcmFrameCounter);             // Monitor the incoming bytes for start and length bytes
zedf9p_status_t processRTCM(uint8_t incoming) __attribute__((weak));                                                        // Given rtcm byte, do something with it. User can overwrite if desired to pipe bytes to radio, internet, etc.
zedf9p_status_t processRTCM_v(uint8_t incoming);                                                                    // Given rtcm byte, do something with it. User can overwrite if desired to pipe bytes to radio, internet, etc.
zedf9p_status_t processUBX(uint8_t incoming, ubxPacket *incomingUBX, uint8_t requestedClass, uint8_t requestedID);          // Given a character, file it away into the uxb packet structure
zedf9p_status_t processUBXpacket(ubxPacket *msg);                                                                           // Once a packet has been received and validated, identify this packet's class/id and update internal flags

// Send I2C/Serial/SPI commands to the module

zedf9p_status_t calcChecksum(ubxPacket *msg);                                                                                     // Sets the checksumA and checksumB of a given messages
zedf9p_status_t sendCommand(ubxPacket *outgoingUBX, uint16_t maxWait, bool expectACKonly); // Given a packet and payload, send everything including CRC bytes, return true if we got a response
zedf9p_status_t sendI2cCommand(ubxPacket *outgoingUBX, uint16_t maxWait);
zedf9p_status_t sendSerialCommand(ubxPacket *outgoingUBX);
zedf9p_status_t sendSpiCommand(ubxPacket *outgoingUBX);

zedf9p_status_t printPacket(ubxPacket *packet, bool alwaysPrintPayload); // Useful for debugging

// After sending a message to the module, wait for the expected response (data+ACK or just data)

zedf9p_status_t waitForACKResponse(ubxPacket *outgoingUBX, uint8_t requestedClass, uint8_t requestedID, uint16_t maxTime);   // Poll the module until a config packet and an ACK is received, or just an ACK
zedf9p_status_t waitForNoACKResponse(ubxPacket *outgoingUBX, uint8_t requestedClass, uint8_t requestedID, uint16_t maxTime); // Poll the module until a config packet is received
...
bool pushRawData(uint8_t *dataBytes, size_t numDataBytes, bool push_raw_data_stop);
