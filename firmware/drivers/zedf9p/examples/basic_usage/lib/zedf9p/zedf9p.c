...
static zedf9p_status_t zedf9p_parse_ubx_byte(zedf9p_t *dev, const uint8_t byte) {
    ...
    switch (parse_state) {
        ...
        case UBX_STATE_LENGTH2:
            ...
            dev->current_message.length = expected_length;
            calculated_ck_a += byte;
            calculated_ck_b += calculated_ck_a;
...
static zedf9p_status_t zedf9p_handle_message(zedf9p_t *dev, const ubx_message_t *message) {
    ...
    if (message->msg_class == UBX_CLASS_NAV) {
        ...
        // RAWX message parsing is more complex due to variable number of measurements
        if (message->length >= 16U) {  // Minimum RAWX header size
            memcpy(&dev->rawx, message->payload, 16U);  // Copy header

            // Calculate number of measurements
            const uint16_t meas_data_len = message->length - 16U;
            const uint16_t meas_size = 32U;  // Size of each measurement
            const uint8_t num_meas = (uint8_t)(meas_data_len / meas_size);

            if (num_meas <= 32U) {  // Maximum measurements we can store
                dev->rawx.num_meas = num_meas;
                memcpy(dev->rawx.meas, &message->payload[16], num_meas * meas_size);
                dev->rawx_valid = true;
