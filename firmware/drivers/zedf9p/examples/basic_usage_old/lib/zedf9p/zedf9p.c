 ...
 */
static zedf9p_status_t zedf9p_init_i2c(zedf9p_t *dev, const uint8_t address) {
...
static zedf9p_status_t zedf9p_write_data(const zedf9p_t *dev, const uint8_t *data, const size_t len) {
    ...
    if (dev->io.interface_type == ZEDF9P_INTERFACE_I2C) {
        return dev->io.i2c.i2c_write(dev->i2c_address, data, len);
        success = dev->io.i2c.i2c_write(dev->i2c_address, data, len);
...
zedf9p_status_t zedf9p_send_ubx_message(zedf9p_t *dev, const uint8_t msg_class,
    ...
    if (!dev || !dev->initialized) return ZEDF9P_ERR_NOT_INITIALIZED;

    if (payload_len > ZEDF9P_MAX_PAYLOAD_SIZE) return ZEDF9P_ERR_BUFFER_OVERFLOW;
