...
static void zedf9p_verify_checksum_manual(zedf9p_t *dev, const ubx_message_t *message) {
...
static zedf9p_status_t zedf9p_handle_message(zedf9p_t *dev, const ubx_message_t *message) {
    ...
    if (message->msg_class == UBX_CLASS_NAV) {
        ...
        if (dev->io.debug_print) {
            char debug_buf[128];
            sprintf(debug_buf, "RAWX Handler: Length=%d", message->length);
            dev->io.debug_print(debug_buf);

            if (message->length >= 12) {
                uint8_t num_meas = message->payload[11];
                sprintf(debug_buf, " NumMeas=%d ExpectedSize=%d\n", num_meas, 16 + (num_meas * 32));
            } else {
                sprintf(debug_buf, " (too short for header)\n");
            }
            dev->io.debug_print(debug_buf);
        }
        ...
        if (message->length >= 16U) {
            ...
            if (message->length == expected_length && num_meas <= 32U) {
                ...
                dev->rawx_valid = true;
                if (dev->io.debug_print) {
                    dev->io.debug_print("RAWX message processed successfully\n");
                }
                // ... callback handling ...
            } else {
                if (dev->io.debug_print) {
                    char debug_buf[128];
                    sprintf(debug_buf, "RAWX validation failed: Length=%d Expected=%d NumMeas=%d\n",
                           message->length, expected_length, num_meas);
                    dev->io.debug_print(debug_buf);
                }
                return ZEDF9P_ERR_INVALID_MESSAGE;
            }
            ...
            if (dev->io.debug_print) {
                dev->io.debug_print("RAWX message too short for header\n");
            }
            return ZEDF9P_ERR_INVALID_MESSAGE;
        }
